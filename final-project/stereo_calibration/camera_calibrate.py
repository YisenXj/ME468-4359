...
class StereoCalibration(object):
    ...
    def read_images(self, cal_path):
        ...
        for i, fname in enumerate(images_right):
            ...
            (thresh, im_bw_l) = cv2.threshold(gray_l, 35, 255, cv2.THRESH_BINARY )
            (thresh, im_bw_r) = cv2.threshold(gray_r, 35, 255, cv2.THRESH_BINARY )
            res_l = np.uint8(im_bw_l)
            res_l=255-res_l
            res_r = np.uint8(im_bw_r)
            res_r=255-res_r
            ###
            cv2.imshow(images_left[i]+"bw", res_l)
            cv2.waitKey(500)
            cv2.imshow(images_right[i]+"bw", res_r)
            cv2.waitKey(500)
            ret_l, corners_l = cv2.findChessboardCorners(res_l, (7, 7), None)
            ret_r, corners_r = cv2.findChessboardCorners(res_r, (7, 7), None)
