...
camera.PushFilter(sens.ChFilterCameraNoisePixDep(0,0.02,0.02))
if(not os.path.exists(os.path.join(output_dir,"camera2/"))):
                    os.mkdir(os.path.join(output_dir,"camera1/"))
camera.PushFilter(sens.ChFilterSave(os.path.join(output_dir,"camera2/")))
...
while (time < tend) :
    ...
    x_pos=northRad*rEarth
    y_pos=-np.cos(gps_data[1]/np.pi*180)*rEarth*eastRad  
