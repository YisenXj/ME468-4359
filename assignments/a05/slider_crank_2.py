...
chrono.SetChronoDataPath('/home/me468/chrono/build/data/')
...
prismatic_ball = chrono.ChLinkLockPrismatic()
prismatic_ball.SetName("prismatic_ball")
prismatic_ball.Initialize(ground,ball,chrono.ChCoordsysD(chrono.ChVectorD(5.5, 0, 0), z2x))
system.AddLink(prismatic_ball)
  #### -------------------------------------------------------------------------
  #### EXERCISE 2.4
  ...
  #### -------------------------------------------------------------------------
tsda_ball= chrono.ChLinkTSDA()
tsda_ball.SetName("tsda ball")
tsda_ball.Initialize(ground,ball,True,chrono.ChVectorD(6.5,0,0),chrono.ChVectorD(0,0,0))
tsda_ball.SetSpringCoefficient(50.0)
tsda_ball.SetDampingCoefficient(5.0)
tsda_ball.SetRestLength(1.0)

spring_tsda = chrono.ChPointPointSpring(0.05, 80, 15)
tsda_ball.AddAsset(spring_tsda)
system.AddLink(tsda_ball)
...
while (application.GetDevice().run()):
    ...
    application.DoStep()
    if system.GetChTime() > 0.05:
        system_time.append(system.GetChTime())
        slider_xPosition.append(slider.GetPos().x)
        motor_torque.append(engine_ground_crank.GetMotorTorque())
...
ax1.plot(system_time,slider_xPosition)
ax1.set(ylabel='position [m]',xlabel='time [s]')
ax1.set_title("slider position")
ax1.grid()
ax2.plot(system_time,motor_torque)
ax2.set(ylabel='Torque [Nm]',xlabel='time [s]')
